...
#define TCR_IRGN_NC      (0 << 8)        // 不可缓存

#define TCR_IRGN_WBWA    (1 << 24)       // 写回写分配（高位）
#define TCR_IRGN_WT      (2 << 8)        // 写通
#define TCR_IRGN_WBNWA   (3 << 8)        // 写回不写分配
#define TCR_IRGN_MASK    (3 << 8)        // 内部缓存属性掩码

// 外部缓存属性
#define TCR_ORGN_NC      (0 << 10)       // 不可缓存
#define TCR_ORGN_WBWA    (1 << 26)       // 写回写分配（高位）
#define TCR_ORGN_WT      (2 << 10)       // 写通
#define TCR_ORGN_WBNWA   (3 << 10)       // 写回不写分配
#define TCR_ORGN_MASK    (3 << 10)       // 外部缓存属性掩码

// 共享属性
#define TCR_SHARED_NON   (0 << 12)       // 不共享
#define TCR_SHARED_OUTER (2 << 12)       // 外部共享
#define TCR_SHARED_INNER (3 << 28)       // 内部共享（高位）

// 粒度设置
#define TCR_TG0_4K       (0 << 14)       // 4KB粒度
#define TCR_TG0_64K      (1 << 14)       // 64KB粒度
#define TCR_TG0_16K      (2 << 14)       // 16KB粒度

// TTBR1禁用
#define TCR_EPD1_DISABLE (1 << 23)       // 禁用TTBR1

// TCR保留位
#define TCR_EL1_RSVD     (1UL << 6 | 1UL << 35 | 1UL << 63)  // EL1保留位
#define TCR_EL2_RSVD     (1UL << 31 | 1UL << 23)             // EL2保留位
#define TCR_EL3_RSVD     (1UL << 31 | 1UL << 23)             // EL3保留位
